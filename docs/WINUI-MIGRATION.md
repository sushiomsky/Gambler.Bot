...
- `BetHistoryPage`: native history surface with persisted SQLite loading, search/outcome filtering, and CSV export.
...
- `dotnet test .\Platforms\Gambler.Bot.WinUI.Tests\Gambler.Bot.WinUI.Tests.csproj -c Release` succeeds with 26 tests covering runtime safety, session state, settings persistence, insight diagnostics, SQLite bet history reading, history filtering, and CSV export.
